...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add moving averages, ratios, Bollinger Bands, and safe trend indicators.
    ...
    for h in horizons:
        if len(df) >= h:
            # Moving average
            ma = df['Close'].rolling(h, min_periods=1).mean().fillna(df['Close'])
            df[f'Close_MA_{h}'] = ma

            # Close ratio (safe Series conversion)
            close_series = df['Close'].squeeze()
            ma_series = df[f'Close_MA_{h}'].squeeze()
            df[f'Close_Ratio_{h}'] = (close_series / ma_series).fillna(1.0)

            # Trend feature (only if Target exists)
            if 'Target' in df.columns:
                trend = df['Target'].shift(1).rolling(h, min_periods=1).sum()
                df[f'Trend_{h}'] = trend.fillna(trend.mean())
            else:
                df[f'Trend_{h}'] = 0  # safe default
